...
const reducer = handleActions({
    [actionTypes.SET_ORIGINNAL_LIST_ORIGIN_TABLE]: {
    ...
    [actionTypes.MODIFY_ORIGIN_LIST_ORIGIN_TABLE]: {
        success: (state, action) => {
            const { dataInners } = action.payload
            return state.set('datasource', Immutable.fromJS(dataInners))
        }
    },
    [actionTypes.SET_ORIGIN_LIST_HEADERS_ORIGIN_TABLE]: (state, action) => state.set('headers', Immutable.fromJS(action.payload))
}, inintialState)
